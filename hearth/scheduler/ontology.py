...
def lookup_duration_s(job: Job, machine: Machine, capacity: Optional[dict],
    ...
    Fallback chain:
      0. est_duration_s: direct per-job duration if positive (U1)
      1. model gen-rate: est_out_tokens / expected_gen_tps  (JS7b, only when the job
         names a required_model that the catalog supplies a gen-rate for)
      2. capacity (task_class x node) p90
      3. capacity (tool x node) p90       — tool taken as the job's task_class
      4. DEFAULT_DURATIONS_S[task_class]  — else DEFAULT_DURATIONS_S['default']
