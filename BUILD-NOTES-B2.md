...
The `file` path resolves relative to the **repo root** (not the docs/ dir).

When the resolved value is a **list**, the checker automatically compares its **length**,
not the list itself. This is documented in the module docstring of `check_doc_claims.py`.
...
An expired waiver is not silently dropped — the checker still prints the row but marks it FAIL
so the expired waiver is visible in the output.
...
| CAPABILITY-ROADMAP.html claims S5b capability | (readable) | confirmed ✓ |

## Omitted claim — test-count assertion

The plan notes that CAPABILITY-ROADMAP.html (~line 103) claims "110/110 tests". This claim
was intentionally omitted from the registry because:

1. The plan spec says: "The test claim is NOT checkable via this registry — omit it and note
   the omission in BUILD-NOTES-B2.md."
2. The test count is not a projection output — it's a historical snapshot baked into the HTML.
   After stream B2, the suite grows to 130 tests, so the claim is already stale.
3. There is no knowledge/*.json field tracking the live test count to check against.

If a test-count claim becomes desirable in the future, it would require either (a) a projection
that writes the last-known test count to knowledge/, or (b) running the suite inside the checker
(which would make the checker a test runner, not a gate — wrong abstraction).

## Waiver seeded

`roadmap-first-capability` is waived with `expires: null` per the plan spec. Reason: the
...
- **Clause 1**: `doc-claims.json` and `doc-claims-waivers.json` are authored objects (statements
  of human intent/will), not projections. The checker reads projection outputs (knowledge/*.json)
  but never writes them.
- **Clause 2**: Both registry and waivers carry audit trail fields (`author`, `created`,
  `reason`).
- **D18 determinism**: `check_doc_claims.py` is a gate, not a projection. The plan explicitly
  permits comparing waiver expiry to today's UTC date in a gate. No wall-clock timestamps are
  written to any derived output.
- **No silent caps**: expired waivers remain visible in the output table as FAIL, not silently
  dropped.
