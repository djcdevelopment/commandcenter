...
def fleet_callers() -> dict:
    path = Path(env("AM4_FLEET_CALLERS_FILE", str(Path.home()/'.config/am4-fleet/fleet-callers.json')))
    if not path.is_file():
        return {}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def authorized(self) -> bool:
        ...
        self.fleet_caller = "legacy"
        dedicated = hermes_token()
        if dedicated and hmac.compare_digest(supplied, f"Bearer {dedicated}"):
            self.hermes_caller = True
            self.fleet_caller = "hermes"
            return True
        for caller, path in fleet_callers().items():
            key = Path(path).read_text().strip()
            if key and hmac.compare_digest(supplied, f"Bearer {key}"):
                self.hermes_caller = True
                self.fleet_caller = caller
                return True
    ...
    def proxy_chat_completions(self) -> None:
        ...
        port = b["port"]
        lease = admission(b)
        deadline = time.monotonic() + float(env("AM4_ADMISSION_WAIT_S", "120"))
        while not lease.acquire(blocking=False):
            readable, _, _ = select.select([self.connection], [], [], 0)
            if readable and not self.connection.recv(1, socket.MSG_PEEK):
                return  # A disconnected waiter must never start model work later.
            ...
            if self.hermes_caller and requested:
                try:
                    record_attempt(attempt_id, started_at, alias, state, bytes(response_capture),
                                   completed and status == 200 and not disconnected.is_set(), status,
                                   self.fleet_caller)
                except Exception as exc:
...
def record_attempt(attempt_id, started_at, alias, state, response, succeeded, status, caller="hermes"):
    usage = None
    documents = []
    try:
    ...
    receipt = {"schema":"hermes.physical-attempt.v1", "accounting_owner":"direct",
        "execution_mode":"external", "run_id":"uncorrelated-"+caller+"-"+attempt_id,
        "caller_id":caller,
