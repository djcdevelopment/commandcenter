---
name: rung
description: Pick the rung and model for a task from the authored family evidence, then emit the ready-to-run local_generate call. Invoke explicitly with /rung when asked "which rung / what should run this / rung for this task", and before any offload where the rung matters.
---

# rung — which rung should run this, and on what evidence?

Uses the HEARTH MCP server at `http://127.0.0.1:8710/mcp`. Explicit invocation only:
choosing a rung is a deliberate step, not something to slip into an unrelated task.
...
into one `mcp__hearth__local_generate(...)` call.
...
- **The recommendation is the only source of a model name.** The authored family
  ...
  `mcp__hearth__recommend_rung(...)`. A model name you remember is not evidence.
...
```
mcp__hearth__recommend_rung(task_family=<name>, prompt_bytes=<n>)
```
...
```
mcp__hearth__query_rung_state(rung=<backend_hint, or "omen-arc" when the hint is absent>)
```
...
`mcp__hearth__queue_status()` is available as a read-only look at the door's own task
lane if you want it; it is not part of this decision and it dispatches nothing.

### 4. Emit the call

```
mcp__hearth__local_generate(prompt=..., task_family=<name>[, backend=<backend_hint>, model=<model_id>][, files=[...]])
```
...
- `mcp__hearth__rotation_load(...)`, `mcp__hearth__rotation_unload(...)`,
  `mcp__hearth__rotation_window(...)` — loading or unloading a model, and the named
  window that has to contain it, are the operator's rotation lane. A recommendation
  never opens one.
- `mcp__hearth__submit_task(...)` — dispatching fleet work is the conductor's, not
  this skill's.
- `mcp__hearth__masters_pet(apply=True)` — never apply a change from inside a
  recommendation.
