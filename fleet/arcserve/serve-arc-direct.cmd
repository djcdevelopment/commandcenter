@echo off
rem serve-arc-direct.cmd -- the PRE-CUTOVER launcher, kept verbatim as the rollback for the
rem 2026-09-03 llama-swap cutover (ADR-0045 / plan P13). To roll back: copy this file over
rem serve-arc.cmd, drop heartharrc-maintenance.stop, run ArcServeRestart (stop-only), remove
rem the sentinel, then schtasks /Run /TN ArcServeBoot. Everything below this header is the
rem serve-arc.cmd of commit c6370b0, unchanged.
rem ------------------------------------------------------------------------------------------
