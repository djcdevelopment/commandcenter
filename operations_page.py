...
def collect():
    ...
    # Define the two renderer paths
    renderer_paths = [
        'fleet/jev/operations_page.py',
        'fleet/jev/capacity_html.py'
    ]
    
    # Compute SHA256 hashes for the renderer source files
    ...
    for path in renderer_paths:
        ...
        if file_path.exists():
            with open(file_path, 'rb') as f:
                content = f.read()
            renderer_source_files[path] = hashlib.sha256(content).hexdigest()
        else:
            # If file is missing, omit it from the dict
            pass
    
    # Capture Git worktree status for the renderer paths
    try:
        result = subprocess.check_output([
            'git', 'status', '--porcelain=v1', '--untracked-files=all', '--', *renderer_paths
        ], cwd=ROOT, text=True)
        renderer_worktree_status = result.splitlines()
    except (subprocess.CalledProcessError, OSError):
        # If command fails, treat as unavailable
        renderer_worktree_status = None
...
def render(document):
    ...
    intro = '<section><h2>What controls what</h2>' + table(
        ['Layer', 'Access and control', 'Observation / feedback'], [
            ['Hearth gateway', 'Scoped inference, backend pins, ta[REDACTED] routing; planning and build-request tools',
    ...
    # Add Renderer provenance section immediately before Planning and observation limits
    intro += '<section><h2>Renderer provenance</h2>'
    
    renderer_source_files = document.get('renderer_source_files')
    if isinstance(renderer_source_files, dict) and renderer_source_files:
        # Display paths and hash values in a table
        rows = [[path, text(hash_val)] for path, hash_val in renderer_source_files.items()]
        intro += table(['Renderer path', 'SHA256'], rows)
    else:
        intro += '<p>Renderer source hashes unavailable.</p>'
    
    # Handle renderer_worktree_status
    status = document.get('renderer_worktree_status')
    if isinstance(status, list) and all(isinstance(line, str) for line in status):
        if status:
            # Non-empty list: show each status line escaped
            rows = [[text(line)] for line in status]
            intro += table(['Local Git differences'], rows)
        else:
            intro += '<p>No local Git differences reported for these two renderers.</p>'
    else:
        intro += '<p>Renderer Git state unavailable.</p>'
    
    intro += '<p>HEAD is a repository reference, not proof that these working files match that commit.</p>'
    intro += '</section>'
