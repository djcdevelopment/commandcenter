...
- **opt it into a specific run via the `builders` CCMETA field above.** As of 2026-07-04 (finding
  #1, conductor commit `63935ee`) an explicit allow-list **OVERRIDES** `exclude_from_build_pool`:
  `_select_builders()` re-admits a node trimmed only by the exclusion flag when it is named in the
  allow-list AND is `mcp_ready` with the `worker` role AND passes a live SSH health probe. So
  `{"builders": ["cc-builder-4"]}` dispatches to the excluded node for that one run; the default
  pool (no allow-list) still excludes it. No more `fleet.json` flip-flop.
  (History: this was WRONG until 2026-07-04 — pour-c2 on 2026-07-03 silently dropped cc-builder-4
  with `requested builders missing from ready set`. See `docs/adr/0003-allowlist-overrides-exclusion.md`.)
