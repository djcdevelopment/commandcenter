...
the fan-out incident (`e287059`), `hearth/health/gaps.py`, `hearth/toolsurface/{preflight,remediate}.py`,
...
- **Sense (rules-first, no NPU yet).** `hearth/health/gaps.py` holds pure coherence checks ("spells")
  ...
  `preflight` HEARTH tool gathers records over SSH and casts them; it is a **pure observer** and never
...
- The watchdog gains a second, deeper sense without a new daemon — it rides the P4 process already trusted
  to self-heal. `preflight`/`remediate`/`watchfire` extend it; they do not replace it.
...
- The observer/actor split (`preflight` never mutates; `remediate` is the only mutator) keeps the sense
