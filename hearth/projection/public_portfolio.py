...
BOUNDARY_WITH_RESEARCH_AND_EXTERNAL = (
    "HEARTH gateway and execution ledgers, including instrumented direct local inference, "
    "plus research runs harvested from server logs and run records"
)
RESEARCH_LIMITATION = (
...
def validate_public_snapshot(snapshot: dict[str, Any]) -> None:
    ...
    try:
        ...
        if set(snapshot) - {"external_inference", "research_runs"} != required:
...
def build_snapshot(
    ...
    *,
    ...
    if execution["external_inference"] is not None:
        ...
        payload["coverage"]["boundary"] = (
            BOUNDARY_WITH_RESEARCH_AND_EXTERNAL
            if research is not None
            else "HEARTH gateway and execution ledgers, including instrumented direct local inference"
        )
