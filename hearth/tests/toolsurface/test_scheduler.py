...
class CapacityLookupTests(TestCase):
    ...
    def test_est_duration_s_overrides_capacity(self) -> None:
        # When est_duration_s is set, it takes precedence over capacity lookup.
        capacity = {
            "contract_version": "capacity.v1",
            "evidence_watermark": None,
            "buckets": [
        ...
        machine = _machine("local-a", "local", 0.0)
        # Should return 42.0 (est_duration_s) not 2.0 (capacity p90)
        self.assertEqual(lookup_duration_s(job, machine, capacity), 42.0)

    def test_est_duration_s_none_falls_through_to_capacity(self) -> None:
        # When est_duration_s is None, fall through to existing behavior.
        capacity = {
            "contract_version": "capacity.v1",
            "evidence_watermark": None,
            "buckets": [
        ...
        machine = _machine("local-a", "local", 0.0)
        # Should use capacity p90: 2000ms -> 2.0s
        self.assertEqual(lookup_duration_s(job, machine, capacity), 2.0)
