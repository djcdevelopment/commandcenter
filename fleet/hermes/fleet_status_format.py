...
# Scheduler extension: OMEN candidate 428aaed; exact count-type corrections
# and integration: Codex. The subsequent JEV retry did not dispatch a worker.
def format_status(state) -> str:
    ...
    if isinstance(scheduler, dict):
        ...
        if type(active_work_val) is int and active_work_val >= 0:
            active_work = str(active_work_val)

        pending_reviews_val = scheduler.get('pending_reviews')
        if type(pending_reviews_val) is int and pending_reviews_val >= 0:
