...
def summarize_decision(record):
    ...
    for judgment in judgments:
        ...
        if not isinstance(cid, str) or re.fullmatch(r"[A-Za-z0-9_-]{1,80}", cid) is None or cid in seen_candidates:
            return {
                "available": False,
                "reason": "missing_or_malformed_decision"
            }
        ...
        if type(score) not in (int, float) or not (0 <= score <= 3) or not math.isfinite(score):
            return {
                "available": False,
                "reason": "missing_or_malformed_decision"
            }
        ...
        if type(ambiguity) not in (int, float) or not (0 <= ambiguity <= 1) or not math.isfinite(ambiguity):
            return {
                "available": False,
                "reason": "missing_or_malformed_decision"
            }
