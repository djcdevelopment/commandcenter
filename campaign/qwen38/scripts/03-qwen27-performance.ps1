...
function Record-Quarantine([string]$Cell, [string]$Reason) {
    [ordered]@{ timestamp = (Get-Date).ToString('o'); cell = $Cell; reason = $Reason } |
...
try {
    ...
    foreach ($topology in @('qwen27-dual-production', 'qwen27-replica-production')) {
    ...
    foreach ($mtpEnabled in $mtpModes) {
        ...
        foreach ($slotDepth in @(8192, 16384)) {
            foreach ($aggregateConcurrency in @(1, 2, 4, 8, 12, 16, 24)) {
                ...
                try {
                    ...
                    Record-Quarantine -Cell $cell -Reason $_.Exception.Message
                    break
                } finally {
                    & (Join-Path $PSScriptRoot 'server-control.ps1') -Action Stop
                }
    ...
    Complete-Q38Receipt -Receipt $receipt -Status passed -Detail 'Production, MTP, throughput-knee, and context-residency matrices completed; infeasible cells quarantined.'
} catch {
