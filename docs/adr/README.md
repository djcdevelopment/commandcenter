...
| [0027](0027-gateway-dispatches-are-not-observations-yet.md) | Gateway dispatches are not observations until we say what they observe | Accepted (2026-07-30), option B built; first capability re-earned; addendum 2026-09-03: gate 2 opened on `omen-swap`; door calls never reach gate 1 (bridged as the tool name) — whether they should is registered, not decided; addendum 2026-09-04: **decided** — bridge `local_generate` only, other tools stay tool-named; a prerequisite (was the producer's silence a defect or a semantics gap?) is still unanswered and gates the implementation |
...
| [0040](0040-serving-lifecycle-is-adopted-not-built.md) | Serving lifecycle is adopted, not built: llama-swap owns process lifecycle | Accepted (2026-08-28); probe-gated, ~60-70% of the proposed controller deleted; Phase 2 landed 2026-09-03: production runs under llama-swap, side seats through the door's rotation window, KV round-trip proven across a swap |
...
| [0042](0042-devices-are-selected-by-type-never-by-index.md) | Devices are selected by type, never by index | Accepted (2026-08-29); the visibility filter had been costing a whole card — removed, not corrected; caught live 2026-09-03: index 2 was the iGPU, the assertion decided; siblings are env 0/1 |
| [0043](0043-the-rung-goes-cold-when-idle.md) | The rung goes cold when idle: keep it warm — and restart it once it has collapsed | Accepted (2026-08-29); >60 s idle costs ~4×, a 1-token ping every 20 s prevents it entirely — supersedes 0041's trigger; held through the 2026-09-03 cutover and rotation proof; rung state implemented (`hearth/health/rungstate.py`); **amended 2026-09-09** — Decision 2 narrowed to *shallow* decay: against a verified idle the rung falls to ~33% and warming does **not** recover it (24 sustained requests: 33%→48%, unstable), while one restart restored 100% |
| [0044](0044-rate-is-not-a-scalar.md) | A rate is not a scalar: baseline epochs, and degraded states are classified | Accepted (2026-08-30); four stable levels in one night — baseline is epoch-scoped, and the restart discriminates ADR-0043 idle collapse from INC-2026-08-30-A; first deliberate epoch boundary stamped 2026-09-03T12:45:25; rotation windows are ledgered exclusion spans |
| [0045](0045-the-scheduler-plans-a-rotating-host.md) | The scheduler plans a rotating host: OMEN is a stateful Machine, llama-swap owns its lifecycle, and every number comes from a receipt | Accepted (2026-09-03); cutover EXECUTED 12:45 (`26a1d66`), production under llama-swap; P11 rotation proof PASSED through the door (placement asserted + BDF-corroborated, KV round-trip); gate 2 OPEN for `omen-swap` (capability_count 2); `rotation_plan` landed (`4ec76db`); tenancy owner OPEN |
| [0046](0046-the-friend-gate-is-the-second-mouth-for-chat-completions.md) | The friend gate is the recognised second mouth for raw chat-completions: an invited friend's own agent may consume the B70s, per-friend key, budgeted | Accepted (2026-09-13); live at omen.tail…/v1 → :8791 → llama-swap |
| [0047](0047-vllm-on-windows-runs-tla-free-one-seat-per-card.md) | vLLM serves the B70s natively on Windows on a TLA-free kernel set, one seat per card, graphs for small decode batches; the CUTLASS-SYCL kernels are a compiler-line problem | Accepted (2026-09-20); 82 tok/s single / 1,196 @64 on one card, 27B at 128k; production unchanged; E3 open |
